 ...
 * @param index
 */
const addQuestionOption = (question: API.QuestionFrameWork, index: number) => {
  if (!question.options) {
    question.options = [];
  }
...
const deleteQuestionOption = (
  question: API.QuestionFrameWork,
  index: number
) => {
  if (!question.options) {
    question.options = [];
...
const loadData = async () => {
  ...
  if (res.data.code === 0 && res.data.data?.records) {
    ...
    console.log("oldQuestion", res);
    if (oldQuestion.value) {
      questionContent.value = oldQuestion.value.content ?? [];
 ...
 * AI 生成题目成功后执行
 */
const onAiGenerateSuccess = (result: API.QuestionFrameWork[]) => {
 ...
 * AI 生成题目成功后执行（SSE）
 */
const onAiGenerateSuccessSSE = (result: API.QuestionFrameWork) => {
  questionContent.value = [...questionContent.value, result];
};
